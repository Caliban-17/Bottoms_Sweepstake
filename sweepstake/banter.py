...
LINES: list[Line] = [
    ...
    Line("Taxi for {loser}. Actually, cancel it. Let them walk.", "loser", when=_started),
    ...
    Line("{leader} top on {leader_pts}, and only because {runner_up} keeps tripping over their own laces.", "leader", when=lambda c: c["margin"] >= 1),
    ...
    Line("{leader_best_club} are {leader_best_pos} and {leader} is walking round like they scouted them personally.", "leader", ("leader_best_club",), when=lambda c: c["leader_best_pos_n"] <= 6),
    ...
    Line("{leader} is {margin} clear. {runner_up} says they're 'still in it'. {runner_up} is not still in it.", "leader", when=lambda c: c["margin"] >= 6),
    ...
    Line("{streak_form}. That's {streak_owner}'s {streak_club}. Read it out loud at their funeral.", "situation", ("streak_club",), when=lambda c: c["streak_n"] >= 3),
    ...
    Line("{best_club} sitting {best_pos}. {best_owner} didn't earn that, but they'll take it and they'll mention it.", "situation", ("best_club",), when=lambda c: c["best_pos_n"] <= 4),
    ...
    Line("{remaining} rounds to go. Plenty of time for {leader} to bottle it. History says they will.", "generic", when=lambda c: _started(c) and c["remaining"] >= 10),
